...
from backend.platform.services import apply_lilith_voice, lilith_voice_profile
...
async def chat(
    ...
    assistant_text = apply_lilith_voice(
        user=current_user,
        user_text=user_message,
        base_text=assistant_text,
        tool_used=tool_used,
        proactive_hint=(
            f"open {str(payload.get('tabHint', '')).replace('tools.', '').replace('.', ' ')}"
            if payload.get("tabHint")
            else None
        ),
    )
    voice_profile = lilith_voice_profile(current_user, user_message)
