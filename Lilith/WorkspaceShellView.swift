...
struct WorkspaceShellView: View {
    ...
    init(initialDestination: WorkspaceDestination = .assistant) {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                // CENTER LAYER: Void + Result Manifestation
                centerLayer
    ...
    // MARK: - Center Layer (Void + Results)

    private var centerLayer: some View {
        ZStack {
            // Living void at center
            VoidCoreView(stateManager: stateManager, viewModel: chatViewModel)

            // Result manifestation
            CenterResultView(viewModel: chatViewModel)
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                HStack(spacing: 10) {
                    Button {
                        ...
                        Label(
                            hasUnlimitedAccess ? "Unlimited" : currencyLabel(credits),
                            systemImage: hasUnlimitedAccess ? "bolt.fill" : "creditcard"
                        )
                        .font(.caption.weight(.semibold))
                        .padding(.horizontal, 12)
                        .padding(.vertical, 10)
    ...
    private var compactHeader: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                Image(systemName: hasUnlimitedAccess ? "bolt.fill" : "creditcard")
                    .font(.system(size: 16, weight: .semibold))
                    .foregroundStyle(.white)
                    .frame(width: 42, height: 42)
                    .background(LilithTheme.surface, in: Circle())
    ...
    private var destinationContent: some View {
        ...
        case .video:
            VideoView { payload in
                chatViewModel.activeResult = payload
            }
        case .image:
    ...
    private func loadChrome() async {
        if authStore.hasUnlimitedFreeAccess {
            isSuperAdmin = true
            currentPlan = "unlimited"
            credits = max(credits, 9_999_999)
        }
        ...
        do {
            ...
            isSuperAdmin = subscription.isSuperAdmin || authStore.hasUnlimitedFreeAccess
    ...
    private var hasUnlimitedAccess: Bool {
        isSuperAdmin || authStore.hasUnlimitedFreeAccess
    }
    ...
    private func loadDataForCurrentDestination() async {
        ...
        case .memory:
            await memoryViewModel.load(token: token)
        case .code, .video, .image, .clone, .account:
            return
        }
    }
...
private struct FinanceWorkspaceView: View {
    ...
    private var walletQuickActionsCard: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 12) {
                Text("Quick actions")
                    .font(.headline)
                    .foregroundStyle(.white)
                HStack(spacing: 10) {
                    walletActionButton("Send", icon: "arrow.up.right") {
                        Task {
                            guard let token = authStore.token else { return }
                            await viewModel.runWalletQuickAction(.send, token: token)
                        }
                    }
                    walletActionButton("Request", icon: "arrow.down.left") {
                        Task {
                            guard let token = authStore.token else { return }
                            await viewModel.runWalletQuickAction(.request, token: token)
                        }
                    }
                    walletActionButton("Add Funds", icon: "plus.circle") {
                        Task {
                            guard let token = authStore.token else { return }
                            await viewModel.runWalletQuickAction(.addFunds, token: token)
                        }
                    }
                    walletActionButton("Withdraw", icon: "banknote") {
                        Task {
                            guard let token = authStore.token else { return }
                            await viewModel.runWalletQuickAction(.withdraw, token: token)
                        }
                    }
                }
    ...
    private var aiFinanceCard: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 12) {
